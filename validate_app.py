...
def validate_imports():
    ...
    try:
        ...
        print(f"✓ Security settings: MAX_UPLOAD_SIZE={settings.max_upload_size}, MAX_INGESTION_ROWS={settings.max_ingestion_rows}")
        print(f"✓ CORS settings: cors_allowed_origins configured")
    ...
    try:
        from app.analysis.llm.client import create_llm_client, RateLimiter
...
def validate_security_features():
    ...
    try:
        from app.api.routers.datasets import ALLOWED_EXTENSIONS, validate_file_upload
...
def validate_cache_invalidation():
    ...
    try:
        ...
        # Check methods exist
        assert hasattr(DatasetService, '_invalidate_analysis_cache'), "Cache invalidation method missing"
        ...
        assert '_invalidate_analysis_cache' in ingest_source, "ingest_reviews doesn't call cache invalidation"
        
        # Check get_dataset_analysis checks staleness
        analysis_source = inspect.getsource(DatasetService.get_dataset_analysis)
        assert '_is_cache_stale' in analysis_source, "get_dataset_analysis doesn't check cache staleness"
